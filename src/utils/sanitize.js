...
// Only http://, https:// and root-relative internal assets are accepted.
// Everything else ? data:, javascript:, vbscript:, blob:, file:, and protocol-relative (//host) ? is rejected
// and replaced with '' to prevent XSS via SVG data URIs or other injection vectors.
function sanitizeUrl(value) {
  if (typeof value !== 'string') return '';
  const normalized = value.replace(/\u0000/g, '').trim();
  if (!normalized) return '';
  if (!/^https?:\/\//i.test(normalized) && !/^\/(?!\/)/.test(normalized)) return '';
  return normalized.replace(/[&<>"'`]/g, (char) => HTML_ENTITY_MAP[char]);
}
