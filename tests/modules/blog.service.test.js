...
const VALID_POST = {
  ...
  blogBannerImage: '/img/banner.webp',
  blogBannerMobileImage: '/img/banner-mobile.webp',
};
...
describe('blogService.createPost', () => {
  ...
  it('creates the default blog page when a post is created before blog settings exist', async () => {
    vi.spyOn(repo, 'findFirst').mockResolvedValue(null);
    vi.spyOn(homeRepo, 'findFirst').mockResolvedValue({ id: 'home-1' });
    vi.spyOn(repo, 'upsertPage').mockResolvedValue({ id: 'bp-default' });
    vi.spyOn(repo, 'findPostBySlug').mockResolvedValue(null);
    const createSpy = vi.spyOn(repo, 'createPost').mockResolvedValue({ id: 'new-post' });

    await blogService.createPost(VALID_POST);

    expect(repo.upsertPage).toHaveBeenCalledWith('home-1', expect.objectContaining({ title: 'Blog' }));
    expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({ blogPageId: 'bp-default' }));
  });

  it('creates the default home and blog pages when neither exists', async () => {
    vi.spyOn(repo, 'findFirst').mockResolvedValue(null);
    vi.spyOn(homeRepo, 'findFirst').mockResolvedValue(null);
    vi.spyOn(homeRepo, 'create').mockResolvedValue({ id: 'home-default' });
    vi.spyOn(repo, 'upsertPage').mockResolvedValue({ id: 'bp-default' });
    vi.spyOn(repo, 'findPostBySlug').mockResolvedValue(null);
    const createSpy = vi.spyOn(repo, 'createPost').mockResolvedValue({ id: 'new-post' });
    ...
    expect(homeRepo.create).toHaveBeenCalledWith(expect.objectContaining({
      blogSectionTitle: 'Blog',
      bottlenecks: [],
      howWeWork: [],
    }));
    expect(repo.upsertPage).toHaveBeenCalledWith('home-default', expect.objectContaining({ title: 'Blog' }));
    expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({ blogPageId: 'bp-default' }));
  });

  it('falls back banner images to the card image when optional banner fields are omitted', async () => {
    vi.spyOn(repo, 'findFirst').mockResolvedValue({ id: 'bp-1' });
    vi.spyOn(repo, 'findPostBySlug').mockResolvedValue(null);
    const createSpy = vi.spyOn(repo, 'createPost').mockResolvedValue({ id: 'new-post' });
    const { blogBannerImage, blogBannerMobileImage, ...post } = VALID_POST;

    await blogService.createPost(post);

    expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({
      blogImage: '/img/post.webp',
      blogBannerImage: '/img/post.webp',
      blogBannerMobileImage: '/img/post.webp',
    }));
  });
...
describe('blogService.updatePost', () => {
  it('updates a post', async () => {
    ...
    await blogService.updatePost('p-1', { title: 'New Title', blogBannerImage: '/img/new-banner.webp' });

    expect(updateSpy).toHaveBeenCalledWith('p-1', expect.objectContaining({
      title: 'New Title',
      blogBannerImage: '/img/new-banner.webp',
    }));
  });
